...
> Current local build/install command: `python3 /Users/tom/Fiji/cloud_extract_eval/build_h_dab_tissue.py --install`. This includes the new selector dependency, tests, a dedicated menu entry, and backup before replacement; prefer it to the historical commands below.
...
"$JDK/bin/javac" --release 8 -cp jars/ij-1.54p.jar \
  ...
  cloud_extract_eval/plugin_src/H_DAB_Tissue_Selector.java \
  cloud_extract_eval/plugin_src/H_DAB_Local_Tissue_Selector.java
